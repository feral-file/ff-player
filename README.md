...
- Player console logs are mirrored to the public FF1 Cloudflare stream as service `player`. `NEXT_PUBLIC_LOG_SAMPLE_RATE` is evaluated once per activity session: `1` uploads every session, `0` disables upload, and a fraction uploads that percentage by chance. Sessions close after five seconds without a log or after one minute of continuous logs. The player keeps arbitrary console arguments local because they may contain playlist, network, or credential data.
- On-device batches go through the loopback-only `feral-controld` `POST /api/logs` route. The stream is disabled for every browser origin except `http://127.0.0.1:8080`. Controld attaches the trusted FF1 device ID and `player` service name before forwarding server-side, so delivery does not depend on public Cloudflare CORS settings. Deploy the matching ffos-user change before this player build; otherwise the bounded retry queue cannot drain.
