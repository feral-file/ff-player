...
- Batches go through the loopback-only `feral-controld` `POST /api/logs` route. Controld attaches the trusted FF1 device ID and `player` service name before forwarding server-side, so delivery does not depend on public Cloudflare CORS settings. Deploy the matching ffos-user change before this player build; otherwise the bounded retry queue cannot drain.
