...
While **repeat-off is intentionally holding** the final artwork slot, the route keeps an explicit hold flag (not “no timer” alone, which is also true during timer gaps and infinite-duration items). In that hold state, a **queued** `setShuffle` or `refreshPlaylist` **promotes the new playlist on cast** (via `applyQueuedPlaylistIfExists`) instead of waiting for another slot-timer tick—because repeat-off would not schedule one. That is **not** “skip the current artwork’s slot”: shuffle still anchors the **current** item at index `0`, so the on-screen artwork can remain the same and a **new** finite slot timer may run for that item before advancing to the next artwork in the updated order.
